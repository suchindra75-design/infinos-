...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div className="bg-[#0e1014] border border-white/[0.1] rounded-2xl w-full max-w-lg shadow-2xl shadow-black/80 overflow-hidden flex flex-col text-zinc-200">
        {/* Modal Header */}
        <div className="px-6 py-4 border-b border-white/[0.08] flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-base text-white font-display">Compartment Thresholds</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto max-h-[75vh] font-body">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-[#00a3ff] uppercase tracking-wider block font-display">
              Cold Compartment Range (°C)
            </label>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <span className="text-[10px] text-zinc-400 font-body">Min (°C)</span>
                <input
                  type="number"
                  step="0.1"
                  value={coldMin}
                  onChange={(e) => setColdMin(parseFloat(e.target.value))}
                  className="w-full bg-[#07080a] border border-white/[0.1] rounded-lg px-3 py-2 text-sm font-data text-white focus:outline-none focus:border-[#00a3ff]"
                  required
                />
              </div>
              <div>
                <span className="text-[10px] text-zinc-400 font-body">Max (°C)</span>
                <input
                  type="number"
                  step="0.1"
                  value={coldMax}
          ...
          {/* Hot Range */}
          <div className="space-y-2">
            <label className="text-xs font-bold text-[#ff6b00] uppercase tracking-wider block font-display">
          ...
          {/* Humidity Range */}
          <div className="space-y-2">
            <label className="text-xs font-bold text-sky-400 uppercase tracking-wider block font-display">
              Relative Humidity Range (%)
            </label>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <span className="text-[10px] text-zinc-400 font-body">Min (%)</span>
                <input
                  type="number"
                  step="0.1"
                  value={humidityMin}
                  onChange={(e) => setHumidityMin(parseFloat(e.target.value))}
                  className="w-full bg-[#07080a] border border-white/[0.1] rounded-lg px-3 py-2 text-sm font-data text-white focus:outline-none focus:border-sky-400"
                  required
                />
              </div>
              <div>
                <span className="text-[10px] text-zinc-400 font-body">Max (%)</span>
                <input
                  type="number"
                  step="0.1"
                  value={humidityMax}
          ...
          <div className="pt-2 border-t border-white/[0.08] flex items-center justify-between">
            <div>
              <span className="text-sm font-semibold text-zinc-100 block font-display">
          ...
          {/* Action Buttons */}
          <div className="pt-4 border-t border-white/[0.08] flex items-center justify-end gap-3 font-body">
            <button
              type="button"
              onClick={onClose}
